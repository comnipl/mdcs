...
pub async fn fetch_commits(
    ...
    let Ok(auth_header_value) =
        HeaderValue::from_str(&format!("Bearer {}", config.github_password))
    else {
    ...
    if let Some(branch_name) = &config.github_branch_name {
        url = format!(
            ...
            config.github_repository_owner_name, config.github_repository_name, branch_name
        );
    } else {
        url = format!(
            "https://api.github.com/repos/{}/{}/commits",
            config.github_repository_owner_name, config.github_repository_name
        );
    };
